...
export const companies = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    postCompany: build.mutation<
      CompanyInfo,
      Partial<CompanyInfo> & { policy_document?: File; policy_file_name?: string }
    >({
      query: (data) => {
        const formData = new FormData();
        ...
        if (data.policy_document) {
          formData.append("files", data.policy_document);
        }

        if (data.policy_file_name) {
          formData.append("file_name", data.policy_file_name);
        }
    ...
    updateCompany: build.mutation<
      CompanyInfo,
      { id: string; data: Partial<CompanyInfo> & { policy_document?: File; policy_file_name?: string } }
    >({
      query: ({ id, data }) => {
        const formData = new FormData();
        ...
        if (data.policy_document) {
          formData.append("files", data.policy_document); // File
        }
        if (data.policy_file_name) {
          formData.append("file_name", data.policy_file_name); // string
        }
